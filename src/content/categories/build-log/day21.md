...
11:00 #ffffff  正午纯白        23:00 #0a0a1a
```
...
核心算法：给定当前小时和分钟，找到两个相邻关键帧，LRUP 每个通道：
...
| 概念 | 理解 |
|------|------|
| **`Intl.DateTimeFormat` 时区指定** | 浏览器原生 API 支持 `timeZone: 'Asia/Singapore'` 格式化任何时区的时间，无需第三方库。配合 `hourCycle: 'h23'` 获取 0-23 小时制 |
| **RGB 线性插值 (LERP)** | `lerp(a, b, t) = a + (b - a) * t`。分别对 R/G/B 三个通道插值，再拼回 `#rrggbb` |
| **Rec.601 亮度公式** | `brightness = 0.299R + 0.587G + 0.114B`。人眼对绿色最敏感，红色次之，蓝色最不敏感。用加权和比简单平均更接近感知亮度 |
| **PMREMGenerator 运行时重建** | 环境贴图可以在场景创建后重建：新建 `PMREMGenerator` → `fromScene(envScene)` → 赋值给 `scene.environment` → 立即 `dispose()`。开销约 1-2ms，每 60 秒重建一次可接受 |
| **CSS `[data-*]` 属性选择器** | `[data-theme="night"]` 的优先级高于 `:root` 中的变量，天然适合作为主题开关。不需要 JS 操作类名，CSS 自己完成切换 |
| **Tailwind 任意值语法** | `bg-[var(--glass-bg)]` 让 Tailwind v4 支持动态 CSS 变量，无需在配置文件中预设颜色。v4 的 `@theme` 指令也可以做同样的事，但 `:root` + 任意值更灵活 |
| **内联阻塞脚本防闪烁** | `<script is:inline>` 在 Astro 中不会被 Vite 打包，保持同步执行。放在 `<head>` 中、CSS 加载之前，确保首次渲染前主题已就位 |
| **`requestAnimationFrame` vs `setInterval`** | RAF 在标签页不可见时自动暂停，适合持续动画；setInterval 即使标签页后台也会运行，浪费性能。60 秒检查一次时间 + RAF 每帧渲染是最优组合 |
